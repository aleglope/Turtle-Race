...
color_mapping = {
    'roja': 'red', 'verde': 'green', 'azul': 'blue', 'naranja': 'orange', 'morada': 'purple'
}
...
def place_bet(player_name, available_colors):
    ...
    if len(available_colors) == 1 and len(players) - sum(1 for p in players.values() if p['bet']) == 1:
        chosen_color = available_colors.pop()
        print(f"Solo queda la tortuga {chosen_color}. {player_name} apostará por ella automáticamente.")
    ...
    while True:
        chosen_color_key = input(
            f"{player_name}, elige una tortuga ({', '.join(color_mapping.keys())}): ").strip().lower()
        if chosen_color_key in color_mapping and color_mapping[chosen_color_key] in available_colors:
